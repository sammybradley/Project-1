/* Engine tests for Fuelcast.  Run with:  node test/engine-test.js
   Loads the browser modules in a VM with a fake localStorage — no DOM, no
...
// Browsers share one global lexical scope across classic scripts; concatenating
// reproduces that so `const FC_DATA` in data.js is visible to store.js.
const src = ['js/data.js', 'js/calc.js', 'js/store.js']
  .map(f => fs.readFileSync(base + '/' + f, 'utf8')).join('\n;\n')
  + '\n;({ FC_DATA: FC_DATA, FC_CALC: FC_CALC, FC_STORE: FC_STORE });';
const { FC_DATA, FC_CALC, FC_STORE } = vm.runInContext(src, sandbox, { filename: 'bundle.js' });
let pass = 0, fail = 0;
