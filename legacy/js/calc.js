...
const FC_CALC = (function () {
  ...
  function showWeight(kg, units, digits) {
    const d = digits === undefined ? 1 : digits;
    return units === 'metric' ? round(kg, d) : round(kgToLb(kg), d);
  }
  function weightUnit(units) { return units === 'metric' ? 'kg' : 'lb'; }
  function showVolume(ml, units) {
    return units === 'metric' ? Math.round(ml) : Math.round(mlToOz(ml));
  }
  function volumeUnit(units) { return units === 'metric' ? 'ml' : 'oz'; }
  ...
  const activityFactor = id => (BASE_ACTIVITY.find(a => a.id === id) || BASE_ACTIVITY[0]).factor;
  ...
  function restingBurn(profile, wearable) {
  ...
  function direction(profile, goal) {
  ...
  function paceOptions(type) { return PACES[type] || PACES.maintain; }

  function pacePct(goal) {
  ...
  function plan(profile, goal, day) {
  ...
  function forecast(profile, goal, planned) {
  ...
  function forecastFromHistory(profile, goal, history) {
  ...
  function goalProgress(startKg, currentKg, targetKg) {
  ...
  function schedule(profile, planned) {
  ...
  function nowAbsolute(sched, nowMin) {
  ...
  function annotateSchedule(sched, day, nowMin) {
  ...
  function kcalPerMinute(met, weightKg) { return met * 3.5 * weightKg / 200; }

  /* Minutes of each activity needed to erase a calorie overshoot. */
  function gapOptions(gapKcal, weightKg, activities) {
  ...
  function weeklyActivityTarget(planned) {
    const base = 150;
  ...
  return {
    KCAL_PER_KG, DAYS_PER_MONTH,
    kgToLb, lbToKg, cmToIn, inToCm, mlToOz, ozToMl,
    showWeight, weightUnit, showVolume, volumeUnit, round, clamp,
    BASE_ACTIVITY, activityFactor, bmr, restingBurn, exerciseBurn,
    PACES, paceOptions, pacePct, direction, plan,
    forecast, forecastFromHistory, humanDuration, goalProgress, addDays,
    minutesOf, fmtTime, schedule, nowAbsolute, annotateSchedule,
    scoreMeal, recommend, kcalPerMinute, gapOptions, weeklyActivityTarget
  };
})();
